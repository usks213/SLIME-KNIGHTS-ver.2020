...
CEnemyBigWing::CEnemyBigWing()
{
	...
	// エリアのサイズ
	m_pViewArea->GetTransform()->SetSize(Float2{ 2000, 2000 });
	m_pAttackArea->GetTransform()->SetSize(Float2{ 2000, 2000 });
	...
	// タイプ
	m_eType = ENEMY::WING_GREEN;

	// UI
	m_pUI->GetTransform()->SetSize(Float2{ 4, 4 });
	...
	//	if (m_bAttack)
		{
			if (m_nMoveInterval < 0)
			{
				for (int i = 0; i < BULLET_NUM; i++)
				{
					...
					//弾の生成
					CBullet::Create(m_transform.GetPos(), force, &m_status, BULLET::GREEN);
					// サウンド
					sm_SE[ENEMY::SE_MUL_SHOT].Play(0.7f);
				}
